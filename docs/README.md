...
| ScanCapa          | Identifies capabilities (behaviors, ATT&CK techniques, MBC objectives) in PE/ELF/Mach-O/.NET binaries using Mandiant's capa (https://github.com/mandiant/capa) and the capa-rules ruleset | `tmp_directory` -- location where `tempfile` will write temporary files (defaults to `/dev/shm/`)<br>`rules` -- path to a capa-rules directory (defaults to `/opt/capa-rules/`)<br>`signatures` -- path to FLIRT `.sig`/`.pat` signatures for library-function identification (defaults to `/opt/capa-sigs/`)<br>`size_limit` -- maximum input file size, in bytes, sent to capa (defaults to `5000000`)<br>`timeout` -- maximum time, in seconds, capa is allowed to run (defaults to `300`)<br>`limit` -- maximum number of matched rules to log in detail (defaults to `200`)                                                                                                                                                                                                                                                                                                                                                                                             |
...
| ScanRetdec        | Decompiles PE/ELF/Mach-O binaries to C pseudocode using the RetDec decompiler (https://github.com/avast/retdec), logs named (resolved) function signatures, and emits the decompiled source as a child file | `tmp_directory` -- location where `tempfile` will write temporary files (defaults to `/dev/shm/`)<br>`size_limit` -- maximum input file size, in bytes, sent to the decompiler (defaults to `5000000`)<br>`timeout` -- value, in seconds, passed to retdec-decompiler's own `--timeout` flag (defaults to `240`)<br>`limit` -- maximum number of named function signatures to log (defaults to `200`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ScanRetdecFileinfo | Collects file format, packer/compiler, and structural metadata using RetDec's static analysis tool (https://github.com/avast/retdec)                                            | `tmp_directory` -- location where `tempfile` will write temporary files (defaults to `/dev/shm/`)<br>`limit` -- maximum number of entries to log from large tables such as imports, exports, sections (defaults to `100`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| ScanVipermonkey   | Emulates VBA macros in document files using ViperMonkey (https://github.com/decalage2/ViperMonkey) to surface deobfuscated strings and runtime actions (e.g. resolved Shell commands) that static macro analysis alone would miss | `tmp_directory` -- location where `tempfile` will write temporary files (defaults to `/dev/shm/`)<br>`vmonkey_bin` -- path to the vmonkey executable, which runs under its own isolated Python 2 interpreter (defaults to `/opt/python2.7/bin/vmonkey`)<br>`size_limit` -- maximum input file size, in bytes, sent to ViperMonkey (defaults to `5000000`)<br>`timeout` -- maximum time, in seconds, ViperMonkey is allowed to run (defaults to `180`)<br>`limit` -- maximum number of recorded actions to log (defaults to `200`)                                                                                                                                                                                                                                                                                                                                                                                             |
